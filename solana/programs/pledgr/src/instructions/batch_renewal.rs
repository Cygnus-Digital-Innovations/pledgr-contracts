use crate::{
    errors::*,
    instructions::process_auto_renewal::{BountyPaid, BountyPaymentFailed},
    state::*,
};
...
pub struct BatchRenewal<'info> {
    ...
    #[account(
        mut,
        seeds = [SUBSCRIPTION_SEED, subscription.subscriber.as_ref(), subscription.subscription_id.as_ref()],
...
pub fn handler(ctx: Context<BatchRenewal>) -> Result<()> {
...
#[event]
pub struct BatchRenewalCompleted {
